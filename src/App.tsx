...
function App() {
  ...
  useEffect(() => {
    refreshInstances();
    searchModpacks("", "relevance", discoverProvider); // Initial fetch
  }, []);
    ...
    try {
      if (provider === "Modrinth") {
        ...
        setModpacks(prev => append ? [...prev, ...newPacks] : newPacks);
          ...
          let packs = data || [];
          if (query.trim()) {
            packs = packs.filter((p: any) => p.name?.toLowerCase().includes(query.toLowerCase()) || p.description?.toLowerCase().includes(query.toLowerCase()));
          }
          setModpacks(packs.map((pack: any) => {
            const safeName = pack.name?.toLowerCase().replace(/[^a-z0-9]/g, "") || "";
            return {
              id: pack.id?.toString() || pack.name,
              title: pack.name,
              author: "ATLauncher Formatter",
              description: pack.description || "An ATLauncher modpack.",
              icon_url: `https://download.nodecdn.net/containers/atl/launcher/images/${safeName}.png`
            }
          }));
        } else if (provider === "Technic") {
          // Their main search endpoint is down and CORS protected. 
          // Using Solder API for base packs as fallback, via allorigins proxy to bypass Chromium CORS
          const res = await fetch(`https://api.allorigins.win/raw?url=${encodeURIComponent('https://solder.technicpack.net/api/modpack')}`);
          const data = await res.json();
          let packs = Object.entries(data.modpacks || {}).map(([slug, name]) => ({
             slug, name: name as string
          }));
          if (query.trim()) {
            packs = packs.filter(p => p.name.toLowerCase().includes(query.toLowerCase()));
          }
          setModpacks(packs.map((pack: any) => ({
            id: pack.slug,
            title: pack.name,
            author: "Technic Platform",
            description: `The official ${pack.name} modpack via Solder API.`,
            // Technic's base CDN uses this specific format for their legacy/base Solder packs:
            icon_url: `https://launcher.technicpack.net/assets/images/modpacks/${pack.slug}.png`
          })));
        } else if (provider === "FTB") {
          let urls: string[] = [];
          if (query.trim()) {
            const safeQuery = encodeURIComponent(query);
            const res = await fetch(`https://api.modpacks.ch/public/modpack/search/100?term=${safeQuery}`);
            const data = await res.json();
            urls = (data.packs || []).map((id: number) => `https://api.modpacks.ch/public/modpack/${id}`);
          } else {
             // For FTB, getting 'all' modpacks gives us 90+ valid IDs directly
            const resAll = await fetch(`https://api.modpacks.ch/public/modpack/all`);
            const dataAll = await resAll.json();
            urls = (dataAll.packs || []).map((id: number) => `https://api.modpacks.ch/public/modpack/${id}`);
          }
          
          // Chunk requests to avoid hitting concurrent connection limits
          const packInfos = await Promise.all(
            urls.map(url => fetch(url).then(r => r.json()).catch(() => null))
          );

          setModpacks(packInfos.filter(p => p !== null && p.status === "success").map((pack: any) => ({
            id: pack.id?.toString(),
            title: pack.name,
            author: pack.authors?.[0]?.name || "FTB Team",
            description: pack.synopsis || pack.description || "An FTB modpack.",
            icon_url: pack.art?.find((a: any) => a.type === "square")?.url || pack.art?.[0]?.url || "https://minotar.net/helm/FTB/100.png",
          })));
      }
    } catch (e) {
                   ...
                   <div className="bg-[#F9FAFB] rounded-[24px] p-8 border border-gray-100 flex flex-col gap-6">
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <label className="block text-sm font-semibold text-gray-700">Allocated Memory (RAM)</label>
                          <span className="text-sm font-bold text-[#111827]">{ramAllocation} MB ({(ramAllocation / 1024).toFixed(1)} GB)</span>
                        </div>
                        <input 
                          type="range" 
                          min="1024" 
                          ...
                          className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-[#111827]" 
                        />
                ...
                <div className="flex items-center gap-4 mb-8">
                  <div className="flex-1 relative">
                    ...
                    <input 
                      ...
                      onKeyDown={(e) => e.key === 'Enter' && searchModpacks(discoverQuery, discoverSort, discoverProvider)}
                  ...
                  <select 
                    ...
                    onChange={(e) => {
                      ...
                      searchModpacks(discoverQuery, e.target.value, discoverProvider);
                    }}
                    className="bg-[#F9FAFB] border border-gray-200 rounded-xl px-4 py-3.5 outline-none font-medium cursor-pointer"
                  >
                  ...
                  <button onClick={() => searchModpacks(discoverQuery, discoverSort, discoverProvider)} className="bg-[#111827] text-white px-6 py-3.5 rounded-xl font-bold shadow-md hover:bg-[#1F2937] transition-all active:scale-95">
                    Search
                  </button>
                      ...
                      {modpacks.map((pack) => (
                        <div key={pack.id} className="bg-[#F9FAFB] border border-gray-100 rounded-[20px] overflow-hidden hover:shadow-lg transition-all group flex flex-col">
                          <div className="h-32 w-full overflow-hidden bg-gray-200 relative">
                               {pack.icon_url ? <img src={pack.icon_url} alt={pack.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 blur-sm opacity-50" onError={(e) => { e.currentTarget.src = `https://minotar.net/helm/${discoverProvider}/100.png` }} /> : null}
                               {pack.icon_url ? <img src={pack.icon_url} alt={pack.title} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 rounded-xl shadow-lg group-hover:scale-110 transition-transform duration-500" onError={(e) => { e.currentTarget.src = `https://minotar.net/helm/${discoverProvider}/100.png` }} /> : null}
                          </div>
